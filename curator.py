...
# TODO: refine with stub for tweepy?
Tweet = Any

Author = namedtuple('Author', ['name', 'screen_name', 'profile_image_url'])


def last_tweet_id_redis_key(list_slug: str) -> str:
    return 'r1:last_tweet_id:{list_slug}'.format(list_slug=list_slug)


def load_last_tweet_id(list_slug: str) -> str:
...
def save_last_tweet_id(last_id: str, list_slug: str) -> None:
...
def get_tweets(api: tweepy.API, list_slug: str, last_tweet_id: str) -> List[Tweet]:
    return api.list_timeline(Settings.TWITTER_SCREEN_NAME, list_slug, since_id=last_tweet_id)


def get_user_from_tweet(tweet: Tweet) -> Author:
    return Author(name=tweet.author.name,
...
def html_img(url: str) -> str:
    return '<img src="{url}"/>'.format(url=url)


def parse_tweet(tweet: Tweet) -> Dict:
    text = tweet.text
